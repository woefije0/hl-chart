...
// style=line(과 stepline)의 실제 동작: na 값을 만나도 끊지 않고 앞뒤 유효한 점을 직선(또는 계단)으로
// 그냥 이어버린다 — splitLineRuns처럼 run마다 시리즈를 쪼개는 게 아니라 na 봉을 배열에서 아예
// 빼버리면 lightweight-charts가 남은 점들을 자동으로 이어준다. (진짜로 끊어야 하는 건 style=linebr뿐.)
function bridgedLinePoints(bars, values, offset){
  const off = offset || 0;
  const shifted = new Array(bars.length).fill(null);
  for(let i = 0; i < bars.length; i++){
    if(values[i] == null || !Number.isFinite(values[i])) continue;
  ...
  const pts = [];
  for(let i = 0; i < bars.length; i++){ if(shifted[i] != null) pts.push({ time: bars[i].time, value: shifted[i] }); }
  return pts;
}
// bridgedLinePoints와 같은 이유로, 봉마다 색이 바뀌는 style=line/stepline도 색이 바뀔 때만 세그먼트를
// 새로 시작하고 na 값은 그냥 건너뛴다(splitColoredLineRuns처럼 na에서 강제로 끊지 않음).
function bridgedColoredLineSegments(bars, values, colors, offset, overrideColor, colorMap, branchKeys){
  const off = offset || 0;
  const shiftedV = new Array(bars.length).fill(null);
  const shiftedColor = new Array(bars.length).fill(null);
  for(let i = 0; i < bars.length; i++){
  ...
  let cur = null;
  for(let i = 0; i < bars.length; i++){
    if(shiftedV[i] == null) continue;
    const c = shiftedColor[i];
    if(c == null){ cur = null; continue; } // color=na는 여전히 그 봉으로 들어오는 선분을 숨긴다
    if(!cur || cur.color !== c){
      const seg = { color: c, points: [] };
      if(cur) seg.points.push(cur.points[cur.points.length - 1]); // 색만 바뀐 경우 자연스럽게 이어붙임
      segments.push(seg);
      cur = seg;
    }
    cur.points.push({ time: bars[i].time, value: shiftedV[i] });
...
function createPlotSeries(chartObj, p, ov, bars, scriptHidden){
  ...
  // style=stepline은 대각선 대신 계단 모양으로 그린다 (lightweight-charts의 lineType 옵션).
  if(p.style === 'stepline') lineOpts.lineType = LightweightCharts.LineType.WithSteps;
  // style=linebr만 na에서 진짜로 끊는다. style=line/stepline은 na를 건너뛰고 앞뒤 점을 이어버린다
  // (실제 TradingView의 style_line 동작 — style_linebr와 구분되는 지점).
  const bridge = p.style !== 'linebr';
  ...
  const segments = uniqueColors.size > 1
    ? (bridge ? bridgedColoredLineSegments : splitColoredLineRuns)(bars, p.values, p.colors, p.offset, ov.color, ov.colorMap, p.branchKeys)
    : (bridge ? [{ color: ov.color || p.color, points: bridgedLinePoints(bars, p.values, p.offset) }].filter(seg => seg.points.length)
