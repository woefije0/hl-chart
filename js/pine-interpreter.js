...
function isPineRefObject(v){ return v instanceof PineArray || v instanceof PineLine || v instanceof PineBox || v instanceof PineLabel || v instanceof PineStruct; }
...
class PineInterpreter {
  ...
  run(bars, inputOverrides){
    ...
    if(!this.n) return { meta: this.meta, plots: [], hlines: [], inputs: this.inputMeta, lines: [], boxes: [], labels: [], shapes: [] };
  ...
  evalRequestSecurity(node){
    const rawArgs = node.args;
    let timeframeVal = '', exprNode = null, lookaheadVal = null, posIdx = 0;
    for(const a of rawArgs){
      if(a.named){
        ...
        if(posIdx === 1) timeframeVal = this.evalExpr(a.value);
