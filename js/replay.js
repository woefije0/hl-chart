...
function updateReplayTimeLabel(){
  ...
  // 예전엔 긴 안내문(replayEnd)의 첫 단어만 잘라 썼는데, 한국어에서는 "불러온"이 되어
  // 뜻이 통하지 않았다. 짧은 배지 문구를 언어별로 따로 둔다(전체 문장은 title 툴팁으로).
  replayTimeLabel.textContent = formatReplayTime(bar) + (atEnd ? ' · ' + t('replayEndBadge') : '');
