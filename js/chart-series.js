...
// 왼쪽 거래량 축을 없앤 뒤(chart-init.js), 대신 이 뱃지 하나로 "현재(또는 마우스로 가리키고 있는
// 캔들의) 거래량 값"을 오른쪽 가격축 위에 표시한다. 거래량 시리즈는 여전히 보이지 않는 left
...
function updateVolumeAxisLabel(){
  ...
  const bar = state.hoverCandle || (state.currentBars.length ? state.currentBars[state.currentBars.length - 1] : null);
  if (!bar) {
    if (lastVolumeLabelKey !== null) { lastVolumeLabelKey = null; volumeAxisLabelEl.style.display = 'none'; }
    return;
  }
