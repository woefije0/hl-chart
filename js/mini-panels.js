...
async function createMiniPanel(containerEl, defaultCandidates){
  ...
  mstate.chart = LightweightCharts.createChart(chartEl, {
    ...CHART_THEME,
    layout: { ...CHART_THEME.layout, fontSize: 10 }, // 미니 패널만 글자를 한 단계 작게
  ...
  async function loadMiniHistory(){
    ...
    try{
      const bars = await loadBars(mstate.coin, mstate.interval); // 메인 차트와 완전히 같은 경로 (candles.js)
      mstate.bars = bars;
  ...
  // 메인 차트/즐겨찾기 가격과 동일하게 util.js의 공용 구독 팩토리를 쓴다
  // (예전엔 여기도 소켓 열기/구독/재연결을 손으로 복사해 갖고 있었다).
  function connectMiniWs(){
    mstate.ws = closeSubscription(mstate.ws);
    if(!mstate.coin) return;
    mstate.ws = createSubscription({
      subscription: { type: 'candle', coin: mstate.coin, interval: mstate.interval },
      channels: 'candle',
      onData: (data) => {
