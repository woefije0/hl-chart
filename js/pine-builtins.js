...
const TOP_LEVEL_BUILTINS = {
  plot: (it, p, n, node) => {
    ...
    if(!rec){ rec = { key, title, color, linewidth, offset, values: new Array(it.n) }; it.plots.set(key, rec); }
    rec.title = title; rec.color = color; rec.linewidth = linewidth; rec.offset = offset;
