...
function applyPlotStyleOverrides(script){
  ...
  const newLinesArr = [];
  active.lastResult.plots.forEach(p => {
    linesArr.filter(l => l._pineKey === p.key).forEach(l => { try{ chartObj.removeSeries(l); }catch(e){} });
    const ov = overrides[p.key] || {};
    const segs = createPlotSeries(chartObj, p, ov, bars, active.pane ? null : script.name, script.hidden);
    newLinesArr.push(...segs);
  });
  if(active.pane) active.pane.plotLines = newLinesArr; else active.overlayLines = newLinesArr;
  const crossMarkers = newLinesArr.reduce((acc, l) => l._pineCrossMarkers ? acc.concat(l._pineCrossMarkers) : acc, []);
  if(active.pane){ active.pane.crossMarkers = crossMarkers; active.pane.renderCrossMarkers(); }
...
// 색이 바뀌는 경계에서는 새 세그먼트의 시작점으로 "색이 바뀌기 직전 점"도 하나 같이 넣어서, 선이
// 끊기지 않고 자연스럽게 이어지도록 한다(겹치는 점 하나로 시각적 연속성을 만듦).
function splitColoredLineRuns(bars, values, colors, offset, overrideColor, colorMap, branchKeys){
  ...
  for(let i = 0; i < bars.length; i++){
    if(shiftedV[i] == null){ cur = null; continue; }
    const c = shiftedColor[i];
    if(!cur || cur.color !== c){
      const seg = { color: c, points: [] };
      if(cur) seg.points.push(cur.points[cur.points.length - 1]); // 이전 점을 겹쳐서 선을 이어붙임
      segments.push(seg);
      cur = seg;
    }
...
function createPlotSeries(chartObj, p, ov, bars, titlePrefix, scriptHidden){
  ...
  if(p.style === 'cross'){
    ...
    for(let i = 0; i < bars.length; i++){
      ...
      markers.push({ time: bars[idx].time, value: v, color: ov.color || pineRemapColorByBranch(p.branchKeys && p.branchKeys[i], p.colors[i], ov.colorMap) || p.color });
    }
    series._pineKey = p.key; series._pineBaseColor = p.color;
...
function activatePineScript(script, result){
  ...
  script.lastError = null;
  const bars = state.currentBars || [];
  const active = { overlayLines: [], overlayPriceLines: [], pane: null, legendEl: null, settingsPanel: null, lastResult: result };
  if(result.meta.overlay){
    result.plots.forEach(p => {
      const ov = (script.plotOverrides || {})[p.key] || {};
      const segs = createPlotSeries(state.chart, p, ov, bars, script.name, script.hidden);
      active.overlayLines.push(...segs);
    });
    result.hlines.forEach(h => {
      if(h.price == null || !Number.isFinite(h.price)) return;
      const pl = state.candleSeries.createPriceLine({ price: h.price, color: h.color, lineWidth: 1, lineStyle: LightweightCharts.LineStyle.Dashed, axisLabelVisible: false, title: '' });
    ...
    result.plots.forEach(p => {
      const ov = (script.plotOverrides || {})[p.key] || {};
      const segs = createPlotSeries(pane.chart, p, ov, bars, null, script.hidden);
      pane.plotLines.push(...segs);
    });
    result.hlines.forEach(h => {
      if(h.price == null || !Number.isFinite(h.price)) return;
...
function createPinePane(id, title){
  ...
  pane.renderCrossMarkers = function(){
    ...
    if(!pane.crossMarkers.length || !pane.plotLines.length) return;
    const anchor = pane.plotLines[0];
    ...
    pane.crossMarkers.forEach(m => {
      ...
      if(y == null) return;
      markerCtx.beginPath();
      markerCtx.arc(x, y, 2, 0, Math.PI * 2);
  ...
  Object.assign(pane, { id, wrap, chart, chartEl, followMain, resizeObs, helperSeries: [], plotLines: [], hlinePriceLines: [], legendItem: wrap.querySelector('.legend-item'), syncPriceScaleWidth, suspend });
  return pane;
}
...
function applyPaneShapeMarkers(pane, shapes, bars){
  if(!pane.plotLines.length) return;
  const anchor = pane.plotLines[0];
  const markers = [];
  shapes.forEach(sh => {
    for(let i = 0; i < bars.length; i++){
