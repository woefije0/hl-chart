...
function toMainSeriesData(bars){
  ...
  if (type === 'VolumeCandle') {
    // Volume Candle은 barcolor()를 색상 힌트로만 받아들이고 진하기는 계속 거래량 비율로
    // 정한다(volumeCandleBaseRgb, chart-types.js) — 그래서 아래의 공용 오버라이드
    // (applyPineBarColors, 불투명 단색으로 덮어씀)는 이 유형에는 적용하지 않는다.
    return applyVolumeColors(displayBars);
  }
  // Pine barcolor()가 지정한 봉 색을 맨 마지막에 덮어쓴다 — 실제 Pine에서도 barcolor()가
  // 캔들 자체의 색보다 우선한다. 라인/영역/Volume Candle은 위에서 이미 처리했다.
  if (typeof applyPineBarColors === 'function') displayBars = applyPineBarColors(displayBars);
  return displayBars;
}

// "데이터는 그대로인데 색만 다시 칠하면 되는" 경우(Pine barcolor 변경 등)를 위한 가벼운 갱신.
// recreateMainSeries()는 시리즈를 지웠다 새로 만들면서 가격축 상태 저장/복원과 가격선 재생성까지
// 하는 무거운 함수라, 색만 바꾸는 데 쓰면 낭비인 데다 화면이 한 번 깜빡인다.
function repaintMainSeries(){
  if (!state.chart || !state.candleSeries || !state.currentBars.length) return;
  state.candleSeries.setData(toMainSeriesData(state.currentBars));
}
...
function recreateMainSeries() {
  ...
  if (type === 'Bar') {
    ...
    // Candlestick / Hollow / HeikinAshi / VolumeCandle — 그리고 "그 외 전부"의 폴백.
    // 예전엔 이 네 가지를 명시적으로 나열해서, 저장된 설정(localStorage)이 손상돼 알 수 없는
    // 값이 들어오면 어떤 분기에도 안 걸려 candleSeries가 null인 채로 아래 setData()에서
    // 터졌다 — 차트가 아예 안 뜨는 상태로 부팅되는 셈. 기본 캔들로 흘려보낸다.
  ...
  // removeSeries()가 이전 시리즈에 붙어있던 가격선(지갑 진입가/청산가 포함)도 같이 지우므로,
  // 새 시리즈가 생긴 지금 다시 그려줘야 한다.
  if (typeof updateWalletPriceLines === 'function') updateWalletPriceLines();
