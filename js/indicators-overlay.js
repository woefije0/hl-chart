...
function computeEmaBandFull(bars){
  ...
  bars.forEach((b, i) => {
    // anchor는 "마지막 봉 직전까지의 상태" 딱 하나만 필요하다. 예전엔 매 봉마다 23개 키짜리
    // 객체를 통째로 복사해서(봉 5천개면 5천 번) 버렸다 — 마지막 봉에서만 한 번 복사한다.
    if(i === lastIdx) anchorBeforeLast = { ...current };
...
function refreshEmaBandFull(){
  ...
  // 라인 4개를 한 번의 순회로 나눠 담는다 (예전엔 같은 배열을 4번 map 했다)
  const pts = { max: [], min: [], avg: [], avg2: [] };
  for(const p of result.points){
    pts.max.push({ time: p.time, value: p.max });
    pts.min.push({ time: p.time, value: p.min });
    pts.avg.push({ time: p.time, value: p.avg });
    pts.avg2.push({ time: p.time, value: p.avg2 });
  }
  ['max', 'min', 'avg', 'avg2'].forEach(key => state.emaSeries[key].setData(pts[key]));
...
// srcArr: 이미 만들어둔 소스 값 배열이 있으면 넘겨서 재사용한다. 호출하는 쪽(BB/MACD/MA리본)이
// 어차피 캐시용으로 같은 배열을 따로 만들고 있었는데, 여기서 또 map을 돌면 같은 계산을
// 봉 개수만큼 두세 번 반복하는 셈이었다 (MACD는 fast/slow/캐시로 3번).
function computeMASeries(bars, length, type, source, srcArr){
  const n = bars.length;
  const src = srcArr || bars.map(b => bbSourceValue(b, source));
...
function computeStdevSeries(bars, length, source, srcArr){
  const n = bars.length;
  const src = srcArr || bars.map(b => bbSourceValue(b, source));
  const out = new Array(n).fill(null);
  for(let i = length - 1; i < n; i++){
...
function refreshBBFull(){
  ...
  const basisArr = computeMASeries(bars, length, maType, source, srcArr);
  const stdevArr = computeStdevSeries(bars, length, source, srcArr);
...
function refreshMaRibbonFull(){
  ...
  maRibbonLineIds().forEach(id => {
    ...
    const cfg = state.maRibbonSettings[id];
    // srcArr/volArr는 증분 갱신(updateMaRibbonLive)이 라인별로 직접 덮어쓰고 push하는 배열이라
    // 라인끼리 공유하면 안 된다. 대신 computeMASeries에 넘겨줘서 같은 값을 두 번 만들지는 않게 한다.
    const srcArr = bars.map(b => bbSourceValue(b, cfg.source));
    const arr = computeMASeries(bars, cfg.length, cfg.type, cfg.source, srcArr);
    const pts = [];
    for(let i = 0; i < bars.length; i++){ if(arr[i] != null) pts.push({ time: bars[i].time, value: arr[i] }); }
    series.setData(pts);
    calc[id] = { srcArr, volArr: bars.map(b => b.volume || 0), maArr: arr };
  });
