...
function pineTokenize(source){
  ...
  for(let li = 0; li < lines.length; li++){
    ...
    if(parenDepth === 0){
      ...
      //   os := high[len] > upper ? 0
      //     : low[len] < lower ? 1 : os
      // 들여쓰기 깊이만으로는 if/for 같은 블록 시작과 구별이 안 되므로, 문장의 맨 앞에는
      // 절대 올 수 없는 ':' 하나로만 좁혀서 감지한다(오탐 위험이 없음).
      let nextIdx = li + 1;
      while(nextIdx < lines.length && /^[ \t]*:/.test(lines[nextIdx])){
        li = nextIdx;
  ...
  finishFuncDecl(callExpr, line, isMethod){
    ...
    for(let i = 0; i < rawArgs.length; i++){
      ...
      if(!a.named && a.value.type === 'Ident' && this.isTypeWord(a.value.name) && i + 1 < rawArgs.length){
        ...
        continue; // 타입 수식어 — 실제 이름은 다음 인자에 있다
      }
