...
// ---------- request.security_lower_tf()용: 구간 전체의 1분봉 페이지네이션 수집 ----------
// fetchHistory 한 번은 최대 500개(=500분)치만 주므로, endTimeMs를 앞으로 당겨가며 여러 번
// 호출해서 [fromSec, toSec] 구간을 채운다. 실제 Pine의 request.security_lower_tf도 조회 가능한
// 과거 intrabar 개수에 상한이 있어서(오래된 메인 봉일수록 인트라바 데이터가 없음), 여기서도
// capCandles로 상한을 두고 그보다 오래된 구간은 그냥 비워둔다(호출부에서 빈 배열로 처리).
async function fetchLowerTf1mRange(coin, fromSec, toSec, capCandles){
  capCandles = capCandles || 20000;
  const MAX_REQUESTS = 60;
  let endMs = (toSec + 60) * 1000;
  const seen = new Set();
  let collected = [];
  for(let i = 0; i < MAX_REQUESTS; i++){
    let history;
    try{ history = await fetchHistory(coin, '1m', endMs); }
    catch(e){ break; }
    if(!history || !history.length) break;
    const chunk = history.map(toChartCandle).filter(c => !seen.has(c.time));
    ...
    if(earliest <= fromSec || collected.length >= capCandles) break;
    endMs = earliest * 1000;
  }
  collected.sort((a, b) => a.time - b.time);
  if(collected.length > capCandles) collected = collected.slice(collected.length - capCandles);
  return collected.filter(c => c.time >= fromSec - 60 && c.time <= toSec + 60);
}
...
// ---------- 봉 한 묶음 불러오기 (REST -> 차트에 바로 넣을 수 있는 배열) ----------
// native/agg 분기, 합성 인터벌의 기간 계산 규칙, 정렬/중복 제거/이상치 필터가 전부 여기 모여 있다.
// 예전엔 selectPair / loadMoreHistory / 미니 패널 세 곳이 이 흐름을 조금씩 다르게 복사해서
//갖고 있었고, 실제로 fillGaps가 한쪽에만 적용돼서 과거로 스크롤한 구간만 비어 보이는 버그가 났다.
// endTimeMs를 주면 그 시점 "이전" 구간을 가져온다(뒤로 스크롤 시 과거 데이터 추가 로딩).
async function loadBars(coin, interval, endTimeMs){
  const meta = INTERVAL_META[interval];
  let bars;
  if(meta.kind === 'agg'){
    // 서버 요청은 base(1m)로, 기간 계산은 목표 인터벌 기준으로 — fetchHistory 주석 참고
    const history = await fetchHistory(coin, meta.base, endTimeMs, interval);
    bars = fillGaps(buildAggCandles(history.map(toChartCandle), meta.ms), meta.ms / 1000);
  }else{
    const history = await fetchHistory(coin, interval, endTimeMs);
    bars = history.map(toChartCandle);
  }
  bars = dedupeByTime(bars);
  // 값이 이상한(NaN, 0 이하, high<low 등) 캔들이 섞여 있으면 걸러내고 개수를 남긴다
...
function pushOrUpdateBar(bar){
  ...
  else { arr.push(bar); isNewBar = true; }
  ...
  updateRsiLive(bar);
  updateMacdLive(bar);
  // Pine으로 가져온 지표는 무겁게 스크립트를 통째로 다시 실행해야 해서 매 틱마다는 못 하고,
  // 실제로 새 봉이 하나 확정될 때만(=barstate.isconfirmed 시점에 맞춰) 다시 계산한다.
  if(isNewBar && typeof refreshAllPineScripts === 'function') refreshAllPineScripts();
}
...
// 증분 갱신으로 바꿨다.
// Pine barcolor()로 지정된 색이 있으면 실시간 갱신 봉에도 그대로 입힌다.
// (Pine 스크립트 자체는 새 봉이 확정될 때만 다시 실행되므로, 형성 중인 봉은 보통 색이 없다)
function withPineBarColor(bar) {
  const c = typeof pineBarColorAt === 'function' ? pineBarColorAt(bar.time) : null;
  return c ? { ...bar, color: c, borderColor: c, wickColor: c } : bar;
}
...
function updateSeriesWithCandle(c) {
  if (state.chartType === 'HeikinAshi') {
    ...
    if (ha) { state.candleSeries.update(withPineBarColor(ha)); updateCandlePriceLineColor(ha); }
    ...
    if ((c.volume || 0) > volumeColorMax) {
      ...
      // applyVolumeColor가 barcolor 색상 힌트 + 거래량 알파를 이미 알아서 합치므로(chart-types.js
      // volumeCandleBaseRgb) withPineBarColor로 다시 덮어쓰면 안 된다 — 덮어쓰면 진하기가 사라진다.
    ...
    state.candleSeries.update(withPineBarColor(c));
    updateCandlePriceLineColor(c);
  }
}

// _sub는 "이 합성 봉을 이루는 base(1m) 봉별 최신 거래량"이다. 같은 1m 봉이 형성되는 동안
// 같은 키가 여러 번 갱신되기 때문에 단순 누적이 아니라 덮어쓰기가 필요하다.
// 예전엔 그때마다 Object.values().reduce()로 전체를 다시 합쳤는데(틱마다), 바뀐 한 칸의
// 차이만 더하면 되는 일이라 O(1)로 바꿨다.
function ingestAggCandle(c, ms){
  const bucketSec = Math.floor((c.time * 1000) / ms) * (ms / 1000);
  const vol = c.volume || 0;
  if(!state.liveAgg || state.liveAgg.time !== bucketSec){
    state.liveAgg = { time: bucketSec, open: c.open, high: c.high, low: c.low, close: c.close, volume: vol, _sub: { [c.time]: vol } };
  }else{
    ...
    if(!state.liveAgg._sub){ state.liveAgg._sub = {}; state.liveAgg.volume = 0; }
    const prev = state.liveAgg._sub[c.time] || 0;
    state.liveAgg._sub[c.time] = vol;
    state.liveAgg.volume = (state.liveAgg.volume || 0) + (vol - prev);
  }
...
// 탭 제목: 여러 탭을 열어두고 쓸 때 "HL Chart"라는 고정 문구보다 "지금 어떤 페어를 얼마에
// 보고 있는지"가 훨씬 쓸모 있어서, 가격이 갱신될 때마다 탭 제목 자체를 페어+가격으로 바꾼다.
// 탭이 좁아지면 브라우저가 뒤쪽부터 잘라내므로, 식별에 제일 중요한 페어 심볼을 맨 앞에 둔다.
function updateDocumentTitle(){
  if(!state.coin) return;
  // state.coin은 스팟 페어에서 "@142" 같은 내부 API용 식별자라 그대로 쓰면 안 되고,
  // 사람이 읽는 이름은 state.label(예: PURR/USDC, BTC-PERP)에 따로 있다.
  const name = state.label || state.coin;
  const priceEl = $('lastPrice');
  const priceText = priceEl ? priceEl.textContent : '';
  document.title = (priceText && priceText !== '—') ? `${name}  ${priceText}` : name;
}
