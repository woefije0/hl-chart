...
const macdTimeScaleLink = linkPaneTimeScale(() => state.macdChart); // chart-init.js — RSI와 공용
...
function createMacdChart(){
  if(state.macdChart) return;
  state.macdChart = LightweightCharts.createChart(macdChartEl, panelChartOptions());
  ...
  macdTimeScaleLink.attach();
}
function destroyMacdChart(){
  if(!state.macdChart) return;
  macdTimeScaleLink.detach();
...
function refreshMacdFull(){
  ...
  // 소스 배열을 한 번만 만들어 fast/slow/캐시 세 곳에서 같이 쓴다
  // (예전엔 computeMASeries가 매번 내부에서 bars.map(bbSourceValue)를 다시 돌려서 3번 만들었다)
  const srcArr = bars.map(b => bbSourceValue(b, source));
  const fastArr = computeMASeries(bars, fastLen, oscType, source, srcArr);
  const slowArr = computeMASeries(bars, slowLen, oscType, source, srcArr);
