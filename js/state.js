...
let state = {
  ...
  candleSeries: null,
  volumeSeries: null,        // 거래량 히스토그램 (왼쪽 가격축에 붙지만 축 자체는 숨김 — chart-init.js)
  priceScaleKeepAlive: null, // 오른쪽 가격축에 항상 붙어있는 보이지 않는 더미 시리즈 (chart-init.js 주석 참고)
  ...
  bbSeries: null,
  bbCalc: null,          // 중간 계산 배열 캐시 — updateBBLive가 마지막 봉 한 칸만 갱신할 때 씀
  ...
  maRibbonCalc: null,     // 라인별 중간 계산 배열 캐시 (bbCalc와 같은 용도)
  ...
  macdCalc: null,         // 중간 계산 배열 캐시 (bbCalc와 같은 용도)
  ...
  // ---------- 대형거래(고래) 감지 (하이퍼리퀴드 특화: 공개 trades 구독을 노셔널 기준으로 필터링) ----------
  whaleOpen: store.get('whale_open', false) === true,
  whaleThreshold: store.get('whale_threshold', 50000),
  whaleTrades: [],        // 최신순 { time, isBuy, px, sz, notional, addr } — 최대 WHALE_MAX_ROWS개 유지
  whaleWs: null,          // trades 구독 전용 웹소켓
  ...
  walletSummary: null,    // { totalPositionValue, withdrawable, marginUsed, unrealizedPnl, spotUsdc } — 계좌 잔고 요약
  walletPriceLines: { entry: null, liq: null }, // 현재 보고 있는 코인에 겹쳐 그린 진입가/청산가 라인 (createPriceLine 참조)
  walletTab: 'positions', // 'positions' | 'history' — 새로고침해도 저장 안 함(패널 열 때마다 포지션 탭부터 시작)
  walletFills: [],        // userFills 응답(최신순, 최대 WALLET_FILLS_LIMIT개) — 히스토리 탭에서 보여주는 최근 체결 목록
  walletFillsLoaded: false, // 지금 추적 중인 지갑에 대해 히스토리를 한 번이라도 불러왔는지 (탭 전환 시 중복 조회 방지)
  walletFillsLoading: false,
  walletFillsError: null, // 조회 실패 시 i18n 키
  // ---------- PineScript 지표 가져오기 ----------
  ...
  // ---------- 청산 히트맵 (모델 추정치 — 실제 시장 전체 청산 데이터는 공개 API로 얻을 수 없다) ----------
  liqHeatmapOn: false,     // on/off (새로고침해도 저장 안 함 — 다른 지표와 동일한 관례)
  liqHeatmapHidden: false, // 범례 눈 아이콘으로 임시 숨김
  liqHeatmapSettings: {
    lookbackDays: 21, interval: '1h', halfLifeDays: 5,
    leverageTierRatios: [0.1, 0.25, 0.5, 0.75, 1.0], // maxLeverage 대비 상대 비율 (코인마다 최대 레버리지가 크게 다름)
    // 강도 0 -> 1 보간 색 (롱/숏 공통, 위/아래 위치로만 구분). Coinglass 스타일: 파랑(낮음) -> 초록 -> 연두 -> 노랑(높음)
    gradient: ['#1a3fff', '#00c853', '#caff33', '#fff200'],
    opacity: 0.75,
  },
  liqHeatmapCanvas: null,   // { canvas, ctx } - createLiqHeatmapCanvas()에서 1회 설정
  liqHeatmapRaw: null,      // 레버리지 팬아웃 직후, OI 스케일링 전 원시 결과 { longBins, shortBins, totalRaw }
  liqHeatmapScaled: null,   // 실제 OI(USD)에 맞춰 스케일된 최종 결과 - 렌더링은 이걸 읽는다
  liqHeatmapLastOiUsd: null, // 마지막으로 리스케일한 시점의 OI(USD) - 다음 틱과 비교해 임계값 미만이면 스킵
  liqHeatmapClockWs: null, // 15분봉 구독(전체 재계산 타이밍용 clock) — createSubscription 핸들
};
