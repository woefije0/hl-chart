...
function resolveXForTime(time){
  ...
  let logical;
  if(time > bars[lastIdx].time) logical = lastIdx + (time - bars[lastIdx].time) / intervalSec;
  else if(time < bars[0].time) logical = (time - bars[0].time) / intervalSec;
  else return null;
...
function startOverlayLoop(){
  ...
  const nothingToDraw = !state.drawings.length && !state.drawDraft;
...
const DRAW_TOOLS = ['cursor', 'trendline', 'hline', 'fib', 'rect', 'text'];
