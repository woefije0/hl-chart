...
const rsiTimeScaleLink = linkPaneTimeScale(() => state.rsiChart); // chart-init.js — MACD와 공용
...
function createRsiChart(){
  if(state.rsiChart) return;
  state.rsiChart = LightweightCharts.createChart(rsiChartEl, panelChartOptions());
  ...
  rsiTimeScaleLink.attach();
}
function destroyRsiChart(){
  if(!state.rsiChart) return;
  rsiTimeScaleLink.detach();
